...
def procesar_nomina(request):
    if request.method == 'POST':
        ...
        if nomina_periodo_pago == "":
            return JsonResponse({'success': False, 'message': 'Ingrese una fecha correcta.'})
        else: 
            ...
            for usuario in usuarios:
                ...
                if Nomina.objects.filter(nom_periodo_pago=nomina_periodo_pago, nom_cedula_id=cedula).exists():
                    return JsonResponse({'success': False, 'message': 'La nomina ya existe.'})
                else:
...
def procesar_retardo(request):
    ...
    precio = int(request.POST.get('precio', 0))
    descuento_tipo = request.POST.get('descuento_tipo')
    cantidad_retardo = int(request.POST.get('cantidad_retardo', 0))
    fecha = datetime.now()
      # Convertir a entero
    # Multiplicar retardos por 1000 para obtener el tiempo de retardo en pesos
    des_time_retardo = cantidad_retardo * 5531
    # verificar si existe ya un retardo registrado y si no crear uno
    if Descuento.objects.filter(desc_cedula_id=cedula, desc_periodo_pago>fecha_reporte).exists():
        # Obtener el registro existente      
        descuento = get_object_or_404(Devengado, deveng_cedula_id=cedula, deveng_periodo_pago=fecha_reporte)
        descuento.desc_time_retardo += cantidad_retardo
    else:
        # Se crea un nuevo objeto Devengado con los valores proporcionados
        descuento = Descuento(
            deveng_cedula_id=cedula,
            desc_periodo_pago=fecha_reporte,
            desc_time_retardo=cantidad_retardo,            
        )
        # Se guarda el objeto Devengado en la base de datos
        descuento.save()
