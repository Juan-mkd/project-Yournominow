...
def procesar_nomina(request):
    if request.method == 'POST':
        ...
        if nomina_periodo_pago == "":
            return render(request, 'registrar_nomina.html', {'nomina_periodo_pago': nomina_periodo_pago})
        else: 
        # Parámetro para procesar la nómina para todos los usuarios
    
            # Obtener todos los usuarios
            usuarios = Usuario.objects.all()
            
            # Definir devengados nomina
            nomina_tipo_pago = 'Electronico'
            dias_trabajados = 30
            # Variable para verificar si hay una nómina existente
            ...
            for usuario in usuarios:
                ...
                # Validar el sueldo básico del cargo
                sueldo_minimo = 1400000  # Este valor debe ser ajustado según tu lógica
                if sueldo_basico_cargo <= sueldo_minimo:
                ...
                if Nomina.objects.filter(nom_periodo_pago=nomina_periodo_pago, nom_cedula_id=cedula).exists():
                    ...
                    # deveng_subs_trans = devengados.values_list('deveng_subs_trans', flat=True).first() or 0
                    # deveng_subs_alim = devengados.values_list('deveng_subs_alim',flat=True).first() or 0
                    ...
                    descuento_tipo = descuentos.values_list('desc_tipo_descuento', flat=True).first() or ""
                    descuento_precio = descuentos.values_list('desc_precio', flat=True).first() or 0
                    
                    # Luego, dentro de tu función o método donde tienes el código
...
def procesar_novedad(request):
    if request.method == 'POST':
        ...
        if Devengado.objects.filter(deveng_cedula_id=cedula, deveng_periodo_pago=fecha_periodo).exists():
            ...
            devengado.deveng_periodo_pago = fecha_periodo
            ...
            # Se guarda el nuevo objeto Devengado en la base de datos
            devengado.save()

        # Devolver una respuesta JSON indicando el éxito del registro
        return JsonResponse({'success': True, 'message': 'Registro exitoso!!'})
    else:
        return JsonResponse({'success': False, 'message': 'No es método POST'})
...
def procesar_descuento(request):
    if request.method == 'POST':
        ...
        creditos_libranza = request.POST.get('creditos_libranza')
        cuotas_sindicales = request.POST.get('cuotas_sindicales')
        embargos_judiciales = request.POST.get('embargos_judiciales')
